        ...
        public async Task<List<RecommendComicDto>> GetAll()
        {
            var result = await (from x in _uow.ComicRepository.GetAll().Where(x => x.IsRecommend && x.Status && x.ApprovalStatus == ApprovalStatusComic.Accept)
                                select new RecommendComicDto
                                {
                                    ...
                                    ComicName = x.Name,
                                    NewestChapter = _uow.ChapterRepository.GetAll().Where(y => y.ComicId == x.Id && y.Status && y.ApprovalStatus == ApprovalStatusChapter.Accept).OrderByDescending(x => x.Rank).Any() ? _uow.ChapterRepository.GetAll().Where(y => y.ComicId == x.Id && y.Status && y.ApprovalStatus == ApprovalStatusChapter.Accept).OrderByDescending(x => x.Rank).First().Name : "",
                                    NewestChapterId = _uow.ChapterRepository.GetAll().Where(y => y.ComicId == x.Id && y.Status && y.ApprovalStatus == ApprovalStatusChapter.Accept).OrderByDescending(x => x.Rank).Any() ? _uow.ChapterRepository.GetAll().Where(y => y.ComicId == x.Id && y.Status && y.ApprovalStatus == ApprovalStatusChapter.Accept).OrderByDescending(x => x.Rank).First().Id : 0,
                                    UpdateTime = _uow.ChapterRepository.GetAll().Where(y => y.ComicId == x.Id && y.Status && y.ApprovalStatus == ApprovalStatusChapter.Accept).OrderByDescending(x => x.Rank).Any() ? _uow.ChapterRepository.GetAll().Where(y => y.ComicId == x.Id && y.Status && y.ApprovalStatus == ApprovalStatusChapter.Accept).OrderByDescending(x => x.Rank).First().UpdateTime ?? _uow.ChapterRepository.GetAll().Where(y => y.ComicId == x.Id && y.Status && y.ApprovalStatus == ApprovalStatusChapter.Accept).OrderByDescending(x => x.Rank).First().CreationTime : null
                                }).ToListAsync();
            return result;
        }
        ...
        public async Task<List<RecommendComicDto>> GetListForCarousel()
        {
            var result = await (from x in _uow.ComicRepository.GetAll().Where(x => x.IsRecommend == true && x.Status && x.ApprovalStatus == ApprovalStatusComic.Accept)
                                select new RecommendComicDto
                                {
                                    ...
                                    NewestChapter = _uow.ChapterRepository.GetAll().Where(y => y.ComicId == x.Id && y.Status && y.ApprovalStatus == ApprovalStatusChapter.Accept).OrderByDescending(x => x.Rank).Any() ? _uow.ChapterRepository.GetAll().Where(y => y.ComicId == x.Id && y.Status && y.ApprovalStatus == ApprovalStatusChapter.Accept).OrderByDescending(x => x.Rank).First().Name : "",
                                    NewestChapterId = _uow.ChapterRepository.GetAll().Where(y => y.ComicId == x.Id && y.Status && y.ApprovalStatus == ApprovalStatusChapter.Accept).OrderByDescending(x => x.Rank).Any() ? _uow.ChapterRepository.GetAll().Where(y => y.ComicId == x.Id && y.Status && y.ApprovalStatus == ApprovalStatusChapter.Accept).OrderByDescending(x => x.Rank).First().Id : 0,
                                    UpdateTime = _uow.ChapterRepository.GetAll().Where(y => y.ComicId == x.Id && y.Status && y.ApprovalStatus == ApprovalStatusChapter.Accept).OrderByDescending(x => x.Rank).FirstOrDefault() != null ? _uow.ChapterRepository.GetAll().Where(y => y.ComicId == x.Id && y.Status && y.ApprovalStatus == ApprovalStatusChapter.Accept).OrderByDescending(x => x.Rank).First().UpdateTime ?? _uow.ChapterRepository.GetAll().Where(y => y.ComicId == x.Id && y.Status && y.ApprovalStatus == ApprovalStatusChapter.Accept).OrderByDescending(x => x.Rank).First().CreationTime : null
                                }).ToListAsync();
            return result;
        }
    }
